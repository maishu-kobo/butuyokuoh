    ...
    <div className="space-y-6">
      ...
      {data.monthlySummary.length > 0 && (
        <div className="bg-white rounded-lg shadow p-4">
          <h3 className="font-semibold text-gray-700 mb-3">月別購入履歴</h3>
            ...
            {data.monthlySummary.map((m) => (
              <div key={m.month} className="flex items-center justify-between py-2 border-b last:border-0">
                ...
                <div className="flex items-center gap-4">
                  <span className="text-sm text-gray-500">{m.count}件</span>
      ...
      {/* 購入済みアイテム一覧 */}
      <div>
        <h3 className="font-semibold text-gray-700 mb-3">購入済みアイテム ({data.items.length}件)</h3>
        {data.items.length === 0 ? (
