...
export async function PUT(request: NextRequest) {
  ...
  const { slack_webhook, line_notify_token, notify_on_price_drop, notify_on_target_price } = body;

  const db = getDb();

  // upsert
  db.prepare(`
    INSERT INTO user_notification_settings (user_id, slack_webhook, line_notify_token, notify_on_price_drop, notify_on_target_price)
    VALUES (?, ?, ?, ?, ?)
